...
const Post = ({ post }) => {
  ...
  const { mutate: deletePost, isPending: isDeleting } = useMutation({
    mutationFn: async () => {
      try {
        const res = await api(`/api/posts/${post._id}`, {
          method: "DELETE",
        });
  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    mutationFn: async () => {
      try {
        const res = await api(`/api/posts/like/${post._id}`, {
          method: "POST",
        });
        const data = await res.json();
  ...
  const { mutate: commentPost, isPending: isCommenting } = useMutation({
    mutationFn: async () => {
      try {
        const res = await api(`/api/posts/comment/${post._id}`, {
          method: "POST",
          headers: {
    ...
    <>
      <div className="flex gap-2 items-start p-4 border-b border-gray-700">
        <div className="avatar w-8 h-8">
          ...
          >
            <img src={postOwner.profileImg || "/avatar-placeholder.png"} className="relative z-[-1]"/>
          </Link>
        </div>
